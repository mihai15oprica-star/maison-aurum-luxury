...
export default function ContactPage() {
  return (
    <>
      ...
      <section id="desk" className="relative py-32 md:py-40">
        <div className="container-luxe grid lg:grid-cols-12 gap-12 lg:gap-20">
              ...
              {desks.map((d, i) => (
                <Reveal key={d.city} delay={i * 0.08}>
                  <div className="border-t border-pearl pt-6">
                    ...
                    <p className="font-sans text-[11px] uppercase tracking-[0.3em] text-noir/50 mb-4">{d.role}</p>
                    <a href={`tel:${d.phone.replace(/\s/g, "")}`} className="block font-serif text-xl link-underline mb-1">{d.phone}</a>
                    <a href={`mailto:${d.email}`} className="block font-sans text-sm text-noir/70 link-underline">{d.email}</a>
                    <p className="mt-3 font-sans text-xs text-noir/40">{d.hours}</p>
                  </div>
                </Reveal>
              ))}
            </div>
          ...
          <div className="lg:col-span-7">
            <Reveal delay={0.2}>
              <div className="bg-cream border border-pearl p-8 md:p-12">
      ...
      <section className="relative py-24 md:py-32 border-t border-pearl bg-cream">
        <div className="container-luxe grid md:grid-cols-2 gap-12 items-center">
          <Reveal>
            ...
            <address className="not-italic font-serif text-2xl text-noir/95 leading-snug">{brand.address}</address>
